...
#Region "Initialisation"
    ...
    Private btnAddEnabled As Boolean = True
    Private btnEditEnabled As Boolean = True
    Private btnCopyEnabled As Boolean = True
    Private btnDeleteEnabled As Boolean = True
    Private btnFinishEnabled As Boolean = True
...
#End Region
    ...
    Private Sub CtrlTable_Resize(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Resize
        ...
        btn_Add.Left = 4
        btn_Edit.Left = btn_Add.Left + btn_Add.Width + 4
        btn_Copy.Left = btn_Edit.Left + btn_Edit.Width + 4
        btn_Delete.Left = btn_Copy.Left + btn_Copy.Width + 4
        btn_Approve.Left = btn_Delete.Left + btn_Delete.Width + 4

        With Me.Table
            .Top = BtnPanel.Height + 5 'ToolBar1.Height
            .Left = 0
            .Width = Me.Width '- (Table.Margin.Left + Table.Margin.Right)
            .Height = Me.Height - (BtnPanel.Height + 5) '+ Table.Margin.Bottom + Table.Margin.Top
        End With
        With Me.Panel
            .Top = Panel.Height + 5 'ToolBar1.Height
            .Left = 0
            .Width = Me.Width '- (Table.Margin.Left + Table.Margin.Right)
            .Height = Me.Height - (Panel.Height + 5) '+ Table.Margin.Bottom + Table.Margin.Top
        End With
        'With Me.ToolBar1
        '    .Width = Me.Width
        'End With
    ...
    Public Sub EnableToolbar(ByVal Add As Boolean, ByVal Edit As Boolean, ByVal copy As Boolean, ByVal Delete As Boolean, ByVal Finish As Boolean)
        If Not mDisablebtnAdd Then btnAddEnabled = Add Else btnAddEnabled = False
        If Not mDisablebtnEdit Then btnEditEnabled = Edit Else btnEditEnabled = False
        If Not mDisablebtnCopy Then btnCopyEnabled = copy Else btnCopyEnabled = False
        If Not mDisablebtnDelete Then btnDeleteEnabled = Delete Else btnDeleteEnabled = False
        If Not mDisablebtnFinish Then btnFinishEnabled = Finish Else btnFinishEnabled = False
        SetButtons()
    End Sub

    Private Sub SetButtons()
        Select Case btnAddEnabled
            Case True
                Me.btn_Add.Image = btn.Images(0)
            Case Else
                Me.btn_Add.Image = btn_Grey.Images(0)
        End Select
        Select Case btnEditEnabled
            Case True
                Me.btn_Edit.Image = btn.Images(1)
            Case Else
                Me.btn_Edit.Image = btn_Grey.Images(1)
        End Select
        Select Case btnCopyEnabled
            Case True
                Me.btn_Copy.Image = btn.Images(2)
            Case Else
                Me.btn_Copy.Image = btn_Grey.Images(2)
        End Select
        Select Case btnDeleteEnabled
            Case True
                Me.btn_Delete.Image = btn.Images(3)
            Case Else
                Me.btn_Delete.Image = btn_Grey.Images(3)
        End Select
        Select Case btnFinishEnabled
            Case True
                Me.btn_Approve.Image = btn.Images(4)
            Case Else
                Me.btn_Approve.Image = btn_Grey.Images(4)
        End Select
    End Sub
    ...
    Public Sub DisableButtons(ByVal Add As Boolean, ByVal Edit As Boolean, ByVal copy As Boolean, ByVal Delete As Boolean, ByVal Finish As Boolean)
        ...
        If mDisablebtnAdd Then btnAddEnabled = False
        If mDisablebtnEdit Then btnEditEnabled = False
        If mDisablebtnCopy Then btnAddEnabled = False
        If mDisablebtnDelete Then btnAddEnabled = False
        If mDisablebtnFinish Then btnAddEnabled = False
    End Sub
    ...
    Private Sub ButtonClick(ByVal sender As System.Object, ByVal e As System.EventArgs) _
        Handles btn_Add.MouseUp, _
                btn_Edit.MouseUp, _
                btn_Copy.MouseUp, _
                btn_Delete.MouseUp, _
                btn_Approve.MouseUp
        ...
        Select Case LCase(pb.Name.ToLower)
            Case "btn_add" ' "btnadd"
                If btnAddEnabled Then
                    btn_Add.Image = btn.Images(0)
                    SetAdd()
                End If

            Case "btn_edit" '"btnedit"
                If btnEditEnabled Then
                    btn_Edit.Image = btn.Images(1)
                    SetEdit()
                End If

            Case "btn_copy" ' "btncopy"
                If btnCopyEnabled Then
                    btn_Copy.Image = btn.Images(2)
                    CopyRecords()
                End If

            Case "btn_delete" ' "btndelete"
                If btnDeleteEnabled Then
                    btn_Delete.Image = btn.Images(3)
            ...
            Case "btn_approve" '"btnfinish"
                If btnFinishEnabled Then
                    btn_Approve.Image = btn.Images(4)
                    c = False
    ...
    Private Sub btn_MouseDown(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) _
        Handles btn_Add.MouseDown, _
                btn_Edit.MouseDown, _
                btn_Copy.MouseDown, _
                btn_Delete.MouseDown, _
                btn_Approve.MouseDown
        ...
        Select Case LCase(pb.Name.ToLower)
            Case "btn_add" ' "btnadd"
                If btnAddEnabled Then
                    btn_Add.Image = btn_red.Images(0)
                End If

            Case "btn_edit" '"btnedit"
                If btnEditEnabled Then
                    btn_Edit.Image = btn_red.Images(1)
                End If

            Case "btn_copy" ' "btncopy"
                If btnCopyEnabled Then
                    btn_Copy.Image = btn_red.Images(2)
                End If

            Case "btn_delete" ' "btndelete"
                If btnDeleteEnabled Then
                    btn_Delete.Image = btn_red.Images(3)
                End If

            Case "btn_approve" '"btnfinish"
                If btnFinishEnabled Then
                    btn_Approve.Image = btn_red.Images(4)
                End If
        End Select
