...
Public Class dCls
    ...
    Public Data As New oDictionary(Of String, DatasetObjectBase)
    Private ble As IList
    Public Deleted As New Dictionary(Of String, DatasetObjectBase)
    Public Downloaded As New Dictionary(Of String, DatasetObjectBase)
    Private _ItemType As String = Nothing
    Private DataGrid As DataView

    Public FilterView As New FilteredBindingList(Of CustomerItem)
    'Dim _data
    'Public Property Data() As oDictionary(Of String, DatasetObjectBase)
    '    Get
    '        Return CType(_data, oDictionary(Of String, DatasetObjectBase))
    '    End Get
    '    Set(ByVal value As oDictionary(Of String, DatasetObjectBase))
    '        _Deleted = _data
    '    End Set
    'End Property

    'Dim _Deleted
    'Public Property Deleted() As Dictionary(Of String, DatasetObjectBase)
    '    Get
    '        Return CType(_Deleted, Dictionary(Of String, DatasetObjectBase))
    '    End Get
    '    Set(ByVal value As Dictionary(Of String, DatasetObjectBase))
    '        _Deleted = value
    '    End Set
    'End Property

    'Dim _Downloaded
    'Public Property Downloaded() As Dictionary(Of String, DatasetObjectBase)
    '    Get
    '        Return CType(_Downloaded, Dictionary(Of String, DatasetObjectBase))
    '    End Get
    '    Set(ByVal value As Dictionary(Of String, DatasetObjectBase))
    '        _Downloaded = value
    '    End Set
...
#Region "Initialisation and finalisation"
    ...
    Public Sub New(ByVal ItemType As String)
        ...
        FilterView.SetParent(Me)

        'Try
        Dim t As Type = Type.GetType(ItemType, True)
        ...
        '    _Deleted = Activator.CreateInstance(vcon)
        '    _Downloaded = Activator.CreateInstance(vcon)
        '    _data = Activator.CreateInstance(constructed)

        '    'Dim test1 As DatasetObjectBase
        '    'test1 = New qItem

        '    'Dim test As Dictionary(Of String, DatasetObjectBase)
        '    'test = New Dictionary(Of String, qItem)

        '    Dim _test = Activator.CreateInstance(vcon)
        '    'Deleted = CType(D, Generic.IDictionary(Of String, DatasetObjectBase))
        '    'Deleted = D
...
#Region "File I/O"
    ...
    Public Sub Load(ByVal source As tSource)
        ...
        If Not IsNothing(Deleted) Then Deleted.Clear()
        If Not IsNothing(Downloaded) Then Downloaded.Clear()
            ...
            Case tSource.File
                ...
                If File.Exists(FileName(tFilePath.data)) Then
                    ...
                    If tmp.Length > 0 Then
                        Dim l As String() = Split(Replace(tmp, Chr(10), ""), Chr(13))
...
#Region "Public functions"
    ...
    Public Function GetList() As System.Collections.IList Implements System.ComponentModel.IListSource.GetList
        ...
        If Not (IsNothing(Data)) Then
            With ble
                If Not IsNothing(ble) Then .Clear()
    ...
    Public Sub NewItemFromObject(ByRef ob As Object)
        Dim oba As Object = NewItem(True)
        Dim pi As System.Reflection.PropertyInfo
        For i As Integer = 0 To UBound(Columns)
            pi = oba.GetType().GetProperty(Columns(i))
        ...
        Next
        pi = oba.GetType().GetProperty("id")
        pi.SetValue("id", ob.GetType().GetProperty("id"), Nothing)
