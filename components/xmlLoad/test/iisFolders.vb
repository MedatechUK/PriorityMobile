...
#Region "Priavte Properties"
    ...
    Private webServer As String = String.Format("http://{0}:8080", "redknave")
            ...
            If IsNothing(MapPath) Then
                ...
                With ConfigASHX
                    ...
                    Catch ex As Exception
                        Throw New Exception(String.Format("Could not open Soap Service configuration from {0}/config.ashx. {1}", webServer, ex.Message))
                    End Try

                    Try
                        If String.Compare(.SelectSingleNode("response").Attributes("MachineName").Value.ToString, Environment.MachineName, True) = 0 Then
                            MapPath = .SelectSingleNode("response").Attributes("SoapPath").Value.ToString
                        Else
                            MapPath = String.Format("\\{0}\{1}", _
                                .SelectSingleNode("response").Attributes("MachineName").Value.ToString, _
                                .SelectSingleNode("response").Attributes("SoapPath").Value.ToString).Replace(":", "$")
                        End If
                    Catch ex As Exception
