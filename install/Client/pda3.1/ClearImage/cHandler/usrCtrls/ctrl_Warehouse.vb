...
#Region "Direct Activations"
    ...
    Public Overrides Sub SetNumber(ByRef cSetting As calcSetting)
        ...
        With thisForm
            .CurrentRow("qty") = CStr(CInt(thisForm.CurrentRow("qty")) - cSetting.DNUM)
            ListSort1.Value("qty", ListSort1.SelectedIndex) = .CurrentRow("qty")
            .TableData.EndEdit()
        End With
        'Dim d As DataRowView = ListSort1.RowSelected()
        Dim pe As XmlNode
        pe = xmlForms.FormData.Document.SelectSingleNode( _
            String.Format( _
                ...
                Chr(34), _
                scCurr.boundxPath, _
                ListSort1.Value("name", Current), _
                ListSort1.Value("serial", Current) _
            ) _
        )
        If IsNothing(pe) Then
            ...
            With pe
                ...
                .ChildNodes(pe.ChildNodes.Count - 1).InnerText = ListSort1.Value("name", Current) & "~" & ListSort1.Value("serial", Current)
                .AppendChild(xmlForms.FormData.Document.CreateElement("name"))
                .ChildNodes(pe.ChildNodes.Count - 1).InnerText = ListSort1.Value("name", Current)
                .AppendChild(xmlForms.FormData.Document.CreateElement("des"))
                .ChildNodes(pe.ChildNodes.Count - 1).InnerText = ListSort1.Value("des", Current)
                .AppendChild(xmlForms.FormData.Document.CreateElement("serial"))
                .ChildNodes(pe.ChildNodes.Count - 1).InnerText = ListSort1.Value("serial", Current)
                .AppendChild(xmlForms.FormData.Document.CreateElement("qty"))
                .ChildNodes(pe.ChildNodes.Count - 1).InnerText = cSetting.DNUM.ToString
                ...
                .ChildNodes(pe.ChildNodes.Count - 1).InnerText = ListSort1.Value("serialpart", Current)
        ...
        Else
            With pe
                .SelectSingleNode("qty").InnerText = CStr(CInt(pe.SelectSingleNode("qty").InnerText) + cSetting.DNUM.ToString)
            End With
        End If
        ...
        With xmlForms.FormData
            ...
            xmlForms.SetNodeChanged(partsnode.ParentNode)
    ...
    Private Sub hCalc()
        thisForm.Calc(New calcSetting(CInt(0)))
    End Sub
