                        ...
                        thisForm.Calc( _
                            New calcSetting( _
                                ...
                                String.Format( _
                                    ...
                                    thisForm.CurrentRow("des").ToString _
                                ) _
                            ) _
                        )

                    Case Else
                        Select Case MsgBox(String.Format("Deliver {0} * {1}?", .CurrentRow("tquant"), .CurrentRow("name")), MsgBoxStyle.YesNoCancel)
                            ...
                            Case MsgBoxResult.No
                                CalcType = eCalcType.qty
                                thisForm.Calc(New calcSetting(CInt(0), , , thisForm.CurrentRow("des").ToString))
        ...
        With thisForm
            If cSetting.Result = DialogResult.OK Then
                Select Case CalcType
                    Case eCalcType.cheese
                        If MsgBox(String.Format("Add {0} weighing {1}kg?", .CurrentRow("name"), cSetting.DNUM.ToString), MsgBoxStyle.OkCancel) = MsgBoxResult.Ok Then
                            Dim OrderItem As XmlNode = .FormData.SelectSingleNode(.boundxPath).ParentNode.SelectSingleNode(String.Format(".//part[ordi='{0}']", .CurrentRow("ordi")))
                            Dim remain As Double = 1
                            While remain > 0
                                remain = DeliverOldestLot(OrderItem, remain, cSetting.DNUM)
                                If remain = -1 Then
                                    MsgBox("Insufficient stock.")
                                    Exit While
                                End If
                    ...
                    Case eCalcType.qty
                        Dim OrderItem As XmlNode = .FormData.SelectSingleNode(.boundxPath).ParentNode.SelectSingleNode(String.Format(".//part[ordi='{0}']", .CurrentRow("ordi")))
                        If cSetting.DNUM <= CDbl(OrderItem.SelectSingleNode("tquant").InnerText) Then
                            Dim remain As Double = cSetting.DNUM
                            While remain > 0
                                remain = DeliverOldestLot(OrderItem, remain)
                                If remain = -1 Then
                                    MsgBox("Insufficient stock.")
                            ...
                            End While
                        Else
                            If MsgBox("You are about to deliver more stock than this customer ordered. Are you sure?", MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
                                Dim remain As Double = cSetting.DNUM
                                While remain > 0
                                    remain = DeliverOldestLot(OrderItem, remain)
                                    If remain = -1 Then
