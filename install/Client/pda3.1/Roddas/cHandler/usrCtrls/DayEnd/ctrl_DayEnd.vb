...
#Region "Overrides base Methods"
    ...
    Public Overrides Sub Bind()
        ...
        If MsgBox("Print Rejects Report?", MsgBoxStyle.OkOnly) = MsgBoxResult.Ok Then
            PrintWhat = ePrintWhat.Rejects
            With thisForm.Printer
                If Not .Connected Then
                    .BeginConnect(thisForm.MACAddress)
                    Do While .WaitConnect
    ...
    Public Overrides Sub PrintForm()
        Select Case PrintWhat
            ...
            Case ePrintWhat.Returns
                PrintReturns()
            Case ePrintWhat.Rejects
                printRejects()
        End Select
    End Sub

    Public Sub PrintRejects()
        Dim dv As XmlNode

        With thisForm
            dv = .FormData.SelectSingleNode("pdadata/home")

            Dim headerFont As New PrinterFont(50, 5, 2)
            Dim largeFont As New PrinterFont(30, 0, 3)
            Dim smallFont As New PrinterFont(35, 0, 2)

            Using lblRejects As New Label(thisForm.Printer, eLabelStyle.receipt)
                ...
                For Each reject As XmlNode In rejects

                    rcptRejects.AddRow("", "")
                    rcptRejects.AddRow("Customer:", "Sales Order:")
                    rcptRejects.AddRow(reject.SelectSingleNode("custnumber").InnerText, reject.SelectSingleNode("sonum").InnerText)
                    rcptRejects.AddRow("Reason:", "")
                    rcptRejects.AddRow(reject.SelectSingleNode("nodeliveryreason").InnerText, "")
                ...
                With lblRejects
                    ...
                    'payments rcpt
                    For Each StrVal In rcptRejects.FormattedText
                        .AddText(StrVal, New Point(22, thisForm.Printer.Dimensions.Height), smallFont)
                    Next


                    .AddLine(New Point(350, thisForm.Printer.Dimensions.Height + 10), _
