...
#Region "delivery"
    ...
    Public Function preClose(ByRef thisform As xForm, ByRef delivery As XmlNode, ByRef CloseException As Exception) As dlgCloseDelivery

        CloseException = Nothing

        Dim ItemsDelivered As Boolean = delivery.SelectNodes("parts/part[cquant > 0]").Count > 0
        Dim AllDelivered As Boolean = delivery.SelectNodes("parts/part[tquant > 0]").Count = 0
        ...
        If ItemsDelivered Then
            ...
            If CBool(sig.SelectSingleNode("mandatory").InnerText = "Y") Then
                If Not (sig.SelectSingleNode("image").InnerText.Length > 0) Then
                    CloseException = New Exception("Customer signature is mandatory.")
                    Return Nothing
                End If
            End If
        End If
        ...
        With dlg
            .NoDelivery = Not (ItemsDelivered)
            .PartialDelivery = ItemsDelivered And Not (AllDelivered)
            .CompleteDelivery = AllDelivered

            If Not (ItemsDelivered) Or Not (AllDelivered) Then

                Dim lbl_Reason As System.Windows.Forms.Label = .FindControl("lbl_Reason")
                Dim lbl_Warning As System.Windows.Forms.Label = .FindControl("lbl_Warning")
                Dim NDReason As ComboBox = .FindControl("Reason")

                Select Case ItemsDelivered
                    Case True
                        lbl_Warning.Text = "WARNING: Not all items have been delivered."
                        lbl_Reason.Text = "Please select reason for partial delivery:"
                    Case Else
                        lbl_Warning.Text = "WARNING: No items have been delivered."
                        lbl_Reason.Text = "Please select reason for non - delivery:"
                End Select
                ...
                With NDReason
                    With .Items
                        ...
                        Select Case ItemsDelivered
                            Case True
                                For Each reason As XmlNode In thisform.FormData.SelectSingleNode("pdadata/reasons/partdelivery").SelectNodes(".//reason")
