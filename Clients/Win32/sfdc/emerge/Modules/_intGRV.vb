...
#End Region
    ...
    Public Overrides Sub TableSettings()
        ...
        CtrlTable.RecordsSQL = "SELECT PARTNAME, PARTDES, PORDERITEMS.TBALANCE/1000, 'Goods' " & _
            "FROM PART, PORDERITEMS, PORDERS " & _
            "WHERE PORDERITEMS.PART = PART.PART " & _
            "AND PORDERS.ORD = PORDERITEMS.ORD " & _
            "AND PORDERS.ORDNAME = '%PONAME%'"
    ...
    Public Overrides Sub ProcessForm()
        ...
        Try
            With p
                ...
                .RecordType1 = "PONAME,BOOKNUM,USERLOGIN"
                .RecordType2 = "PARTNAME,TOWARHS,TOLOC,CQUANT,STATUS"
    ...
    Public Overrides Sub EndInvokeData(ByVal Data(,) As String)
        ...
        For i As Integer = 0 To CtrlTable.Table.Items.Count
            If CtrlTable.Table.Items(i).SubItems(0).Text = Data(0, 0) Then
                ...
                Dim exp As Integer = CInt(CtrlTable.Table.Items(i).SubItems(3).Text)
                Dim rcvd As Integer = CInt(CtrlTable.Table.Items(i).SubItems(4).Text)
                Dim add As Integer
                ...
                If add > 0 Then
                    ...
                    If rcvd + add > exp Then
                        If MsgBox("Receiving more than ordered. Continue?", MsgBoxStyle.OkCancel, "Part " & Data(0, 0)) = MsgBoxResult.Ok Then
                            CtrlTable.Table.Items(i).SubItems(4).Text = CStr(rcvd + add)
                        End If
                    Else
                        CtrlTable.Table.Items(i).SubItems(4).Text = CStr(CInt(CtrlTable.Table.Items(i).SubItems(4).Text) + add)
                    End If
