...
if ($result[image_a])
	...
	<img src="assets/images/products/thumbs/'.$result[image_a].'" border="0" alt=""/>
	<span><img src="assets/images/products/large/'.$result[image_a].'"  alt=""/>
	<br/><small>'.$result[text_a].'</small></span></a>');

	if ($result[image_b])
		{
		echo('<a class="thumbnail" href="#thumb">
		<img src="assets/images/products/thumbs/'.$result[image_b].'" border="0" alt=""/>
		<span><img src="assets/images/products/large/'.$result[image_b].'" alt=""/>
		<br/><small>'.$result[text_b].'</small></span></a>');
		}
	...
	echo("<img src='$result[large]' alt='An image of $result[name]' class='main_img'/></div>");
		...
		if ($result[stock] == "NO")
			...
			echo('<p style="color:#F00"><strong>This product is currently out of stock.</strong> <a href="stock_update.php?product_id='.$result[id].'" title="Click here to get an alert when the '.$result[name].' is back in stock">Email me when back in stock</a></p><p><a href="products.php" title="Click here to go back to product list"><img src="assets/images/back_button.gif" alt="Back to product list"/></a></p></form>');
			}
		else
			{
			echo('<a href="products.php" title="Click here to go back to product list"><img src="assets/images/back_button.gif" alt="Back to product list"/></a> <input name="submit" type="image" value="Submit" src="assets/images/add_to_basket.gif" alt="Add to Basket" /></form>');
		...
		// CUT OUT OAK HOUSE IP ADDRESSES
		if ($ip == "195.112.48.143" || $ip == "213.210.21.186")
			{
			echo("<!-- Oak House User -->");
			}
		else
			{
			$process = mysql_query("insert into SHOP1_report_popular (product, ip, visit) VALUES ('$result[id]', '$ip', '$date_added')");
			}
		}
