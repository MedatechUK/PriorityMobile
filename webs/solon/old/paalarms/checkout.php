...
<?php echo writeShoppingCart(); ?>
  ...
  <div id="main_content">
    ...
    <form method="post" action="checkout.php?check=yes">
      ...
      <p>
        ...
        <select name="deliverymethod">
          <option value="<?echo($POSTAGE_ONE_PRICE);?>" <?php if ($_POST[cover] == $POSTAGE_ONE_PRICE) { echo ("selected='selected'"); } ?>>Royal Mail 1st Class (&#163;<?echo($POSTAGE_ONE_PRICE);?>)</option>
          <option value="<?echo($POSTAGE_TWO_PRICE);?>" <?php if ($_POST[cover] == $POSTAGE_TWO_PRICE) { echo ("selected='selected'"); } ?>>Royal Mail Next Day (&#163;<?echo($POSTAGE_TWO_PRICE);?>)</option>
        </select> <a href="assets/popups/d_and_r.php" onclick="raw_popup('assets/popups/d_and_r.php'); return false" title="Opens in a new window" target="_blank" class="new_window">Delivery information</a><br/>All deleiveries require a signature
      </p>
      ...
      <p class="buttons">
	  <input name="submit" type="image" value="Submit" src="assets/images/continue.gif" alt="Continue" /> 
      </p>
      </fieldset>
    </form>
    ...
    <form method="post" action="https://securetrading.net/authorize/form.cgi" onsubmit="javascript:__utmLinkPost(this);return checkCheckBox(this)">
      <fieldset>
      ...
      <div id="confirm">
		...
		$delivery .= $_POST['title'];
		$delivery .= '|'.$_POST['firstname'];
		$delivery .= '|'.$_POST['surname'];
		$delivery .= '|'.$_POST['street'];
		$delivery .= '|'.$_POST['address2'];
		$delivery .= '|'.$_POST['town'];
		$delivery .= '|'.$_POST['county'];
		$delivery .= '|'.$_POST['postcode'];
		$delivery .= '|'.$_POST['tel'];
		$delivery .= '|'.$_POST['email'];
		$delivery .= '|'.$_POST['deliverymethod'];
		$delivery .= '|'.$_POST['billing'];
...
echo('<p>Telephone: '.$_POST[tel].'<br/>Email: '.$_POST[email].'</p>');
		...
		?></div>
			...
			<input type="hidden" name="orderref" value="PAA / <?php echo date("y-mdHi") ?>"/>
			...
			<input type="hidden" name="callbackurl" value="3"/>
			<input type="hidden" name="failureurl" value="3"/>
...
<p class="buttons">
<input name="submit" type="image" value="Submit" src="assets/images/continue.gif" alt="Continue"/></p>
</fieldset>
 ...
 <?php
    ...
    <form method="post" action="checkout.php?check=yes">
      ...
      <p>
        <label><span>Please select delivery type</span></label>
        <select name="deliverymethod">
          <option value="<?echo($POSTAGE_ONE_PRICE);?>" <?php if ($delivery_items[10] == $POSTAGE_ONE_PRICE) { echo ("selected='selected'"); } ?>>Royal Mail 1st Class (&#163;<?echo($POSTAGE_ONE_PRICE);?>)</option>
          <option value="<?echo($POSTAGE_TWO_PRICE);?>" <?php if ($delivery_items[10] == $POSTAGE_TWO_PRICE) { echo ("selected='selected'"); } ?>>Royal Mail Next Day (&#163;<?echo($POSTAGE_TWO_PRICE);?>)</option>
        </select> <a href="assets/popups/d_and_r.php" onclick="raw_popup('assets/popups/d_and_r.php'); return false" title="Opens in a new window" target="_blank" class="new_window">Delivery information</a><br/>All deleiveries require a signature
      </p>
