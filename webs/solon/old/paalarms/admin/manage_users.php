...
<input name="usrpwd" id="usrpwd" type="text" onkeyup="updatestrength( this.value );"/>&nbsp;<img src="../assets/images/admin/tooshort.jpg" id="strength" alt="" /></label>
</p>
...
elseif($_GET[id])
	{
	$query = mysql_query("select * from SHOP1_admin_user where id='$_GET[id]'");
...
if($_GET[edit])
{
	...
	$process=mysql_query("update SHOP1_admin_user set email='$_POST[email]', name='$title', access_rights='$rights' where id='$_POST[id]'"); 
	...
	$tracker=mysql_query("insert into SHOP1_admin_tracker (name, loggedin, ip, job) values ('$_COOKIE[user_name]', '$tracker_time', '$ip', 'User - updated')");
	}
}
...
if($_GET[added])
{
	...
	$process=mysql_query("insert into SHOP1_admin_user (email, name, pwd, access_rights) values ('$_POST[email]', '$title', '$pwd', '$rights')");
	...
	$tracker=mysql_query("insert into SHOP1_admin_tracker (name, loggedin, ip, job) values ('$_COOKIE[user_name]', '$tracker_time', '$ip', 'User - added ($title)')");
	}
}
...
$query = mysql_query("select * from SHOP1_admin_user where id='$_GET[delete]'");
$result = mysql_fetch_array($query);

$delete_db=mysql_query("delete from SHOP1_admin_user where id='$_GET[delete]'");
if(!$delete_db)
	{
	...
	$tracker=mysql_query("insert into SHOP1_admin_tracker (name, loggedin, ip, job) values ('$_COOKIE[user_name]', '$tracker_time', '$ip', 'User - deleted ($result[name])')");
	}
}
...
/////////////////////////////////////////////////////////
	...
	");
	$query = mysql_query("select * from SHOP1_admin_user where access_rights != 'master' order by name asc");
	while($result = mysql_fetch_array($query))
		{
		if($num == "1")
					{
