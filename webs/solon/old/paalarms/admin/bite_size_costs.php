...
if ($_GET[edit])
	{
	$query = mysql_query("select * from SHOP1_report_monthly where month = '$_GET[edit]' and year = '$_GET[year]'");
    ...
    <form method="post" action="report_monthly.php?edited=ok">
	  ...
	  <p>
        <label for="postage"><span>Postage (£)</span></label>
        <input name="postage" id="postage" type="text" size="10" value="<?php echo("$result[postage]"); ?>"/>
      </p>
      <p>Notes</p>
      <?php
    ...
    <form method="post" action="report_monthly.php?added=ok">
      ...
      <p>
	   ...
	   <p>
        <label for="postage"><span>Postage (£)</span></label>
        <input name="postage" id="postage" type="text" size="10"/>
      </p>
      <p>Notes</p>
      <?php
		$oFCKeditor = new FCKeditor('notes') ;
