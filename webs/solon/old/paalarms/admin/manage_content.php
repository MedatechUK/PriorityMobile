...
if($_GET[id])
	{
	$query = mysql_query("select * from SHOP1_content where id='$_GET[id]'");
...
<div id='admin'>
	<table width='90%'>
		...
		<tbody>
<?php
$r_query = mysql_query("select * from SHOP1_content_archive where friendly='$_GET[id]' order by id desc");
...
if($_GET[edit])
{
	...
	$update = date('YmdHis');
	$process=mysql_query("update SHOP1_content set content='$_POST[content]', pagetitle='$title', metatags='$_POST[metatags]', metadesc = '$_POST[metadesc]', update_time='$update' where id='$_POST[id]'"); 
	
	$process_archive=mysql_query("insert into SHOP1_content_archive (friendly, content, pagetitle, metatags, metadesc, update_time) VALUES ('$_GET[edit]', '$_POST[content]', '$title', '$_POST[metatags]', '$_POST[metadesc]', '$update')");
	...
	$process=mysql_query("insert into SHOP1_admin_tracker (name, loggedin, ip, job) values ('$_COOKIE[user_name]', '$tracker_time', '$ip', 'Content Updated - $_POST[id]')");
	}
}
...
$query= mysql_query("select * from SHOP1_content_archive where id = '$_GET[rollback]'");
$result = mysql_fetch_array($query);

// UPDATE CURRENT CONTENT WITH ARCHIVE INFO

$update = date('YmdHis');
$process=mysql_query("update SHOP1_content set content='$result[content]', pagetitle='$result[pagetitle]', metatags='$result[metatags]', metadesc = '$result[metadesc]', update_time='$update' where id='$result[friendly]'"); 

$process_archive=mysql_query("insert into SHOP1_content_archive (friendly, content, pagetitle, metatags, metadesc, update_time) VALUES ('$result[friendly]', '$result[content]', '$result[pagetitle]', '$result[metatags]', '$result[metadesc]', '$update')");
	...
	$process=mysql_query("insert into SHOP1_admin_tracker (name, loggedin, ip, job) values ('$_COOKIE[user_name]', '$tracker_time', '$ip', 'Content Rollback - $result[friendly]')");
	}
}
	...
	");
	$query = mysql_query("select * from SHOP1_content order by friendly asc");
	while($result = mysql_fetch_array($query))
		{
		...
		echo("<tr class='row2'><td>$result[friendly]</td><td><small>$date</small></td><td>$count</td><td><a href='manage_content.php?id=$result[id]' class='edit_page'>Edit</a></td></tr>");		
		}
