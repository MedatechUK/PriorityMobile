...
for($i=0;$i<count($newArr);$i++){
	...
	while($selRes=mysql_fetch_array($selQry)){
		...
		$itemPricePreTax=($itemPrice*1000)/1200;
...
for($i=0;$i<$noOfProducts;$i++){
	...
	$TAXFORTHISROW=round(($itemsArr[$i]['qty']*$itemsArr[$i]['price']*.2),2);
	...
	$CHECKSUM_SUM=$CHECKSUM_SUM+$CHECKSUM_SUM_LINE;
      ...
      case "3.95":
      		$POSTAGE=3.95;
      		$POSTPRODCODE="DELPOST";
			$PostageDescription="Postage and Packing";
      break;
      case "8.95":
      		$POSTAGE=8.95;
     		$POSTPRODCODE="DELIVERY";
...
$POSTAGEPRETAX=($POSTAGE*1000)/1200;
...
<Order xmlns=\"urn:schemas-basda-org:2000:purchaseOrder:xdr:3.01\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xsi:schemaLocation=\"urn:schemas-basda-org:2000:purchaseOrder:xdr:3.01 http://www.ebis-xml.net/schemas/order-v3.xsd\">
  ...
  <OrderReferences>
    ...
    <SuppliersOrderReference Preserve=\"true\">$xml_orderID</SuppliersOrderReference>
  </OrderReferences>
  <OrderDate>$xml_orderDATETIME</OrderDate>
   <Buyer>
    ...
    <Party>TIHS Limited</Party>
    <Address>
      <AddressLine>Oak House</AddressLine>
      <Street>Groes Lwyd</Street>
      <City>Abergele</City>
      <State></State>
      <PostCode>LL22 7SU</PostCode>
    ...
    <Contact>
      ...
      <Fax></Fax>
      <Email>peter.smith@theitc.co.uk</Email>
      </Contact>
  </Buyer>
  <Delivery>
    <DeliverTo>
...
require("xml_order/mime_mail.inc");
require("xml_order/smtp_mail.inc");
$content_type="application/xml";
$smtp_server = "knight.net-work.net";
$mail = new mime_mail;
//$mail->from = "peter@theitc.co.uk";
//$mail->to = "peter@theitc.co.uk,sage@redlinesecurity.co.uk";

$mail->from = "shop@redlinesecurity.co.uk";
$mail->to = "mike.roberts@net-work.net,peter@theitc.co.uk";
