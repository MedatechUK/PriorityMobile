...
for($i=0;$i<count($newArr);$i++){
	...
	while($selRes=mysql_fetch_array($selQry)){
		...
		$itemPricePreTax=($itemPrice*1000)/1175;
...
$xml_orderDATETIME=date("2007-12-04");
...
for($i=0;$i<$noOfProducts;$i++){
	...
	$TAXFORTHISROW=round(($itemsArr[$i]['qty']*$itemsArr[$i]['price']*.175),2);
...
$POSTAGEPRETAX=($POSTAGE*1000)/1175;
...
// (2007 + 13 x 4 -7 x (CHECKSUM) x (12 + 17 x 3 + 5 * 15.80) = 
$PRODTOT = "15.80";

$CHECKSUM=($xml_orderDATEARR[0]+13*$xml_orderDATEARR[2]-7*($CHECKSUM_SUM) )*($xml_orderDATEARR[1]+17*$noOfProducts+5*$PRODTOT);
