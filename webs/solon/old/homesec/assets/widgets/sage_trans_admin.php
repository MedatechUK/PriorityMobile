...
for($i=0;$i<count($newArr);$i++){
	...
	$selSql="";
	$selSql="SELECT * FROM SHOP2_products WHERE id='".$newArr[$i]['id']."'";
	//echo($selSql);
	$selQry=mysql_query($selSql);
	while($selRes=mysql_fetch_array($selQry)){
		...
		// DISCOUNT FUNCTION
			if ($itemsArr[$i]['qty'] >= 10 && $itemsArr[$i]['qty'] < 20)
				{
				if (!$selRes['dis10'])
					{
					...
					{
					$itemPrice = $selRes['dis10'];
					}
				}
			elseif ($itemsArr[$i]['qty'] >= 20 && $itemsArr[$i]['qty'] < 50)
				{
				if (!$selRes['dis20'])
					{
					$itemPrice=$selRes['price'];
					...
					{
					$itemPrice = $selRes['dis20'];
					}
				}
			elseif ($itemsArr[$i]['qty'] >= 50)
				{
				if (!$selRes['dis50'])
					{
					$itemPrice=$selRes['price'];
...
<Order xmlns=\"urn:schemas-basda-org:2000:purchaseOrder:xdr:3.01\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xsi:schemaLocation=\"urn:schemas-basda-org:2000:purchaseOrder:xdr:3.01 http://www.ebis-xml.net/schemas/order-v3.xsd\">
  ...
  <Delivery>
    <DeliverTo>
      ...
      <Country Code=\"United KingdomR\">United Kingdom</Country>
    </Address>
    <Contact>
...
$mail->from = "hs@redlinesecurity.co.uk";
...
$mail->subject =  "Home Security Store Order ". $filenameXML;
$bodyText="Home Security Store Order";
$mail->body=$bodyText;
