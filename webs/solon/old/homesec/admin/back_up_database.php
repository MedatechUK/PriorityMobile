...
if ($_GET[backup])
	...
	if ($_GET[backup] == "server")
		{
		$backupFile = '/secure/h/homesec/admin/backups/'. $name . date("Y-m-d-H-i-s") . '.gz';
	...
	if ($_GET[backup] == "server")
		...
		$process=mysql_query("insert into $admin_track_table (name, loggedin, ip, job) values ('$_COOKIE[user_name]', '$tracker_time', '$ip', 'Database - backed up')");
		}
	else
		{
...
if ($_GET[delete])
	{
	$backupFile = '/secure/h/homesec/admin/backups/'. $_GET[delete];
	unlink("$backupFile");
	$status = "<div id='status_ok'><p>The file $_GET[delete] has been deleted</p></div>";
	$process=mysql_query("insert into $admin_track_table (name, loggedin, ip, job) values ('$_COOKIE[user_name]', '$tracker_time', '$ip', 'Database - deleted')");
	}

//==========================================================================
//PURGE POPULAR PROCESS
//==========================================================================	
if ($_GET[popular])
	{
	$process=mysql_query("DELETE FROM $report_popular_table");
	if ($process)
		{
		$status = "<div id='status_ok'><p>The Popular Product table has been cleared</p></div>";
		$process=mysql_query("insert into $admin_track_table (name, loggedin, ip, job) values ('$_COOKIE[user_name]', '$tracker_time', '$ip', 'Database - Popular products cleared')");
		}
	else
		{
		$status = "<div id='status_error'><p>There was an error when clearing the data from the Popular product table</p></div>";
		}	
	}
...
if ($_GET[footprints])
	{
	$process=mysql_query("DELETE FROM $footprint_table");
	if ($process)
		{
		$status = "<div id='status_ok'><p>The footprints table has been cleared</p></div>";
		$process=mysql_query("insert into $admin_track_table (name, loggedin, ip, job) values ('$_COOKIE[user_name]', '$tracker_time', '$ip', 'Database - Footprints cleared')");
		}
	else
		{
...
//==========================================================================
//SEARCH TERMS PROCESS
//==========================================================================	
if ($_GET[search_terms])
	{
	$process=mysql_query("DELETE FROM $search_table");
	if ($process)
		{
		$status = "<div id='status_ok'><p>The Search Terms table has been cleared</p></div>";
		$process=mysql_query("insert into $admin_track_table (name, loggedin, ip, job) values ('$_COOKIE[user_name]', '$tracker_time', '$ip', 'Database - Search Terms cleared')");
		}
	else
		{
		$status = "<div id='status_error'><p>There was an error when clearing the data from the Search Terms table</p></div>";
		}	
	}
?>
...
$dir = opendir('/secure/h/homesec/admin/backups/');
$loc = "/secure/h/homesec/admin/backups/";
