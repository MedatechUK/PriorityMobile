...
<p>Solon: 01352 762266<br />Sue: 01352 736117<br />The ITC: 01745 828440</p>
  ...
  <div id="main_content">
    <?php  
//$tracker=mysql_query("insert into $admin_track_table (name, loggedin, ip, job) values ('$_COOKIE[user_name]', '$tracker_time', '$ip', 'Report - Viewed Monthly')");	
...
for ($ycount=1; $ycount<=5; $ycount++)
	{
	$year_query 	= mysql_query("select * from $orders_table where date like '$this_year%' limit 1");
...
if($_GET[edited])
{
	...
	$process=mysql_query("update $report_monthly_table set costs='$costs', notes='$_POST[notes]' where id='$_POST[id]'"); 
	...
	$tracker=mysql_query("insert into $admin_track_table (name, loggedin, ip, job) values ('$_COOKIE[user_name]', '$tracker_time', '$ip', 'Report - Updated Monthly Cost')");	
	}
}
...
{
$costs = str_replace("£", "", $_POST[costs]);
$process=mysql_query("insert into $report_monthly_table (costs, notes, month, year) values ('$costs', '$_POST[notes]', '$_POST[month]', '$_POST[year]')"); 
	...
	$tracker=mysql_query("insert into $admin_track_table (name, loggedin, ip, job) values ('$_COOKIE[user_name]', '$tracker_time', '$ip', 'Report - Added Monthly Cost')");	
	}
}
...
echo("<div id='reports'>
	...
	$month_query = mysql_query("select * from $orders_table where date like '$year%';");
