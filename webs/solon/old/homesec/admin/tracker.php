...
if($_GET[delete])
	{
		...
		$delete_db=mysql_query("delete from $tracker_table where id='$_GET[delete]'");
		//LOG IN TRACKER
		if(!$delete_db)
			{
			echo("<div id='status_error'><p>The log entry could not be removed</p></div>");
			}
		else
			{
			echo("<div id='status_ok'><p>The log entry has been removed</p></div>");
			}
			
	}
$query = mysql_query("SELECT * FROM $tracker_table order by id desc");
...
$result 		= mysql_query("SELECT count(*) FROM $tracker_table");
$total 			= mysql_result($result, 0);
$pagination 	= mysql_query("SELECT * FROM $tracker_table order by id desc LIMIT $start, $display");
...
<thead>
	...
	<th>Options</th>
		...
		//$query = mysql_query("SELECT * FROM $tracker_table$order  LIMIT $page, $limit");
		$numrows = mysql_num_rows($pagination);
		if ($numrows > 0)
		{
